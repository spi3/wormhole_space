...
### Requires Python 3.9 
OvenSpace can run using the [Flask](https://flask.palletsprojects.com/en/2.0.x/) web framework in a Python 3.9 environment. However, if your operating system does not have Python 3.9 installed, you need to install it.
...
```
Create a virtual environment for Python. Make sure you are using Python 3.9, and activate the virtual environment. A detailed explanation is [here](https://docs.python.org/3.9/library/venv.html#creating-virtual-environments).
```
$ python -m venv venv
...
You can use `Server.xml` to run OvenMediaEngine with docker with the command below in the project directory.
```
...
OME_WEBRTC_PROVIDER_ENABLE_TLS = 'false'
OME_WEBRTC_PROVIDER_PORT = '3332'

OME_WEBRTC_PUBLISHER_ENABLE_TLS = 'false'
OME_WEBRTC_PUBLISHER_PORT = '3332'

OME_LLHLS_PUBLISHER_ENABLE_TLS = 'false'
OME_LLHLS_PUBLISHER_PORT = '3332'
...
#### `SITE_HOST`
Set the OvenSpace domain or IP address.

#### `SITE_PORT`
Set the OvenSpace port.
...
- **Default**: `ovenspace.cfg` in the working directory
- **Example**:
  ```bash
  export OVENSPACE_CONFIG=/etc/ovenspace/production.cfg
  python OvenSpace.py
  ```

**Note**: Application will exit with an error if the specified file doesn't exist.
...
When not using TLS, the minimum settings of OvenMediaEngine's `Server.xml` required for OvenSpace is: [Server.xml](./Server.xml)
